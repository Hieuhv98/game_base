...
public class PetCollection : ScriptableObject
{
    ...
    private static PetCollection Instance => instance ? instance : (instance = Resources.Load<PetCollection>(Constant.PET_COLLECTION_PATH));

    public PetDataController pet;
    ...
    public static string GetEggSkinName(int id)
    {
        ...
        for (int i = 0; i < Length; i++)
        {
            if (instance.infos[i].id == id)
            {
                return instance.infos[i].eggSkinName;
            }
        }
    ...
    public static int PickRandomEggShard()
    {
        ...
        for (int i = 0; i < 6; i++)
        {
            if (PetDataController.Instance.CurrentNumberShard(instance.infos[i].id) < instance.infos[i].numberShardBreak)
            {
    ...
    private static (int, int) CheckMaximum(ref List<PetInfo> petCollects)
    {
        ...
        (bool, int) CalculateRate(int id1, int id2)
        {
            ...
            if (PetDataController.Instance.CurrentNumberShard(id2) >= instance.infos[id1].maximumNumberCollects[id2])
            {
        ...
        int rate = 0;
        PetDataController pet = PetDataController.Instance;
